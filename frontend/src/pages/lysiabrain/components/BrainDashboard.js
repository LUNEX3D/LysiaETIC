            ...
            <section style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.6fr 1fr", gap: "1.5rem" }}>
                <Card glow style={{
                    ...
                    <div style={{ display: "flex", gap: "1rem", marginTop: "2rem", flexWrap: "wrap" }}>
                        <Btn glow onClick={onAutoDecide} loading={autoDecideLoading}>{t("dash.auto_decide")}</Btn>
                        <Btn variant="outline" onClick={() => onTabChange("recommendations")}>{t("tab.recommendations")}</Btn>
                        <Btn variant="outline" onClick={onDiagnosis} loading={diagnosisLoading}>{t("dash.diagnosis")}</Btn>
                    </div>
                </Card>
                        ...
                        {(redAlerts.alerts || []).map((alert, i) => (
                            <Card key={i} glow style={{ background: "rgba(248,113,113,0.08)", borderLeft: `5px solid ${T.red}`, padding: "1.5rem" }}>
                                <div style={{ display: "flex", gap: "1.25rem", alignItems: "flex-start" }}>
                                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: T.redDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem", flexShrink: 0 }}>{alert.icon || "⚠️"}</div>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ display: "flex", gap: "0.75rem" }}>
                                            ...
                                            <Btn size="sm" variant="outline" onClick={() => onTabChange("losses")}>Analyze</Btn>
                                        </div>
                                    </div>
                                    {alert.amount > 0 && (
