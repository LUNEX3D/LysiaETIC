...
const MODES = {
    passive: { label: "Pasif Gözlemci", color: C.green, emoji: "🟢", icon: "👁️", gradient: `linear-gradient(135deg, ${C.green}, #059669)`, desc: "Sadece analiz ve öneri", glow: `0 0 20px rgba(0,255,136,0.3)` },
    assisted: { label: "Asistan Mod", color: C.yellow, emoji: "🟡", icon: "🤝", gradient: `linear-gradient(135deg, ${C.yellow}, #d97706)`, desc: "AI önerir, siz onaylarsınız", glow: `0 0 20px rgba(255,204,0,0.3)` },
    autonomous: { label: "Otonom Pilot", color: C.red, emoji: "🔴", icon: "⚡", gradient: `linear-gradient(135deg, ${C.red}, #dc2626)`, desc: "Tam otomatik yönetim", glow: `0 0 20px rgba(255,51,102,0.3)` },
};

const RATINGS = {
    excellent: { color: C.green, label: "Mükemmel", emoji: "🟢" },
    good: { color: C.blue, label: "İyi", emoji: "🔵" },
...
const CYCLE_PHASES = [
    { key: "observe", label: "GÖZLEM", icon: "👁️", color: "#00f0ff", desc: "Veri toplama" },
    { key: "analyze", label: "ANALİZ", icon: "🔬", color: "#a855f7", desc: "Metrik hesaplama" },
    { key: "decide", label: "KARAR", icon: "🧠", color: "#ffcc00", desc: "Strateji belirleme" },
    { key: "act", label: "AKSİYON", icon: "⚡", color: "#00ff88", desc: "Uygulama" },
    { key: "verify", label: "DOĞRULA", icon: "✅", color: "#3b82f6", desc: "Sonuç kontrolü" },
    { key: "learn", label: "ÖĞREN", icon: "📚", color: "#ff61d8", desc: "Hafızaya kaydet" },
];
...
const TypingIndicator = () => (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem" }}>
        ...
        <span style={{ color: C.muted, fontSize: "0.72rem", fontStyle: "italic" }}>AI sinir ağları çalışıyor...</span>
    </div>
);
...
const AIOperatorPanel = () => {
    ...
    const loadStatus = useCallback(async () => {
        try {
            ...
            setError("Operatör durumu yüklenemedi: " + (err.response?.data?.message || err.message));
    ...
    const changeMode = async (mode) => {
        ...
        } catch (err) { setError("Mod değiştirilemedi: " + (err.response?.data?.message || err.message)); }
    };
    ...
    const runCycle = async () => {
        ...
        } catch (err) { setError("Döngü çalıştırılamadı: " + (err.response?.data?.message || err.message)); }
    ...
    const sendChat = async (text) => {
        ...
        try {
            ...
            setChatMessages(prev => [...prev, { role: "ai", content: "Bağlantı hatası. Sunucu erişilebilir olduğundan emin olun. 🔌", ts: new Date().toISOString(), suggestions: ["Tekrar dene"] }]);
    ...
    const TABS = [
        ...
        { id: "alerts", label: `Uyarılar (${alerts.length})`, icon: "🚨", color: criticalAlerts.length > 0 ? C.red : C.green },
        { id: "memory", label: "Hafıza & Öğrenme", icon: "🧬", color: C.purple },
        { id: "chat", label: "AI Sohbet", icon: "💬", color: C.accentAlt },
        { id: "cycle", label: "Döngü Sonuçları", icon: "🔄", color: C.blue },
    ];
    ...
    if (loading) return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: C.bg, gap: "1.5rem" }}>
            ...
            <motion.span animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} style={{ color: C.accent, fontSize: "0.9rem", fontWeight: 700, letterSpacing: "0.1em" }}>NEURAL NETWORK BAŞLATILIYOR...</motion.span>
        </div>
    );
        ...
        <div style={{ width: "100%", minHeight: "100vh", background: `linear-gradient(180deg, ${C.bg} 0%, #080d18 50%, ${C.bg} 100%)`, position: "relative", overflow: "hidden", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
            ...
            <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} style={{ background: `linear-gradient(180deg, rgba(5,10,18,0.95) 0%, rgba(5,10,18,0.8) 100%)`, borderBottom: `1px solid ${C.border}`, padding: "1rem clamp(1rem,4vw,2.5rem)", position: "sticky", top: 0, zIndex: 100, backdropFilter: "blur(20px)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                        ...
                        <div>
                            <h1 style={{ background: `linear-gradient(135deg, ${C.accent} 0%, ${C.accentAlt} 50%, ${C.pink} 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontSize: "clamp(1.1rem,2.5vw,1.5rem)", fontWeight: 900, margin: 0, letterSpacing: "0.02em" }}>LYSIA AI OPERATÖR</h1>
                            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.2rem", flexWrap: "wrap" }}>
                                <Pill color={modeConfig.color}>{modeConfig.emoji} {modeConfig.label}</Pill>
                                <Pill color={rating.color}>{rating.emoji} Sağlık: {healthScore}/100</Pill>
                                {criticalAlerts.length > 0 && <Pill color={C.red}>🚨 {criticalAlerts.length} uyarı</Pill>}
                    ...
                    <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                        ...
                        <NeonButton color={C.accent} onClick={runCycle} disabled={cycleLoading} style={{ background: cycleLoading ? `${C.accent}10` : `linear-gradient(135deg, ${C.accent}20, ${C.accentAlt}20)`, boxShadow: cycleLoading ? "none" : C.neonGlow }}>
                            {cycleLoading ? <><motion.span animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}>⚙️</motion.span> Çalışıyor...</> : <>▶️ Döngü Çalıştır</>}
                        </NeonButton>
                    </div>
                </div>
                {/* Tabs */}
            ...
            <AnimatePresence>{error && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ background: `${C.red}10`, borderBottom: `1px solid ${C.red}25`, padding: "0.6rem clamp(1rem,4vw,2.5rem)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 50 }}>
                    <span style={{ color: C.red, fontSize: "0.8rem", fontWeight: 600 }}>⚠️ {error}</span>
            ...
            <div style={{ padding: "clamp(1rem,3vw,1.75rem) clamp(1rem,4vw,2.5rem)", position: "relative", zIndex: 1 }}>
                ...
                <GlassCard delay={0.05} style={{ marginBottom: "1.25rem" }} glow={pulseActive ? C.neonGlowStrong : undefined}>
                    <SectionTitle icon="🔄" title="AI Otonom Döngüsü" badge={cycleLoading ? "⚡ Çalışıyor..." : cycleResult ? `${cycleResult.durationMs || 0}ms` : "✓ Hazır"} color={cycleLoading ? C.yellow : C.accent} />
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === "overview" && (
                        <motion.div key="ov" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                            <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
                                <StatCard icon="📦" label="Toplam Ürün" value={operatorStatus?.stats?.productCount || 0} color={C.accent} delay={0.1} />
                                <StatCard icon="🛒" label="Sipariş (30g)" value={operatorStatus?.stats?.orderCount || 0} color={C.green} delay={0.15} />
                                <StatCard icon="⏳" label="Bekleyen Öneri" value={operatorStatus?.stats?.pendingRecs || 0} color={C.yellow} delay={0.2} />
                                <StatCard icon="🧠" label="AI Hafıza" value={operatorStatus?.memory?.totalMemories || 0} color={C.purple} delay={0.25} />
                                <StatCard icon="🚨" label="Uyarı" value={alerts.length} color={alerts.length > 0 ? C.red : C.green} delay={0.3} />
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1.25rem", marginBottom: "1.25rem" }}>
                                {/* Health */}
                                <GlassCard delay={0.15} glow={`0 0 30px ${rating.color}15`}>
                                    <SectionTitle icon="💎" title="İşletme Sağlığı" color={rating.color} />
                                    <div style={{ display: "flex", alignItems: "center", gap: "1.5rem", justifyContent: "center", flexWrap: "wrap" }}>
                                        ...
                                        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                            {[{ l: "Ürün", v: operatorStatus?.stats?.productCount || 0, i: "📦", c: C.accent }, { l: "Sipariş", v: operatorStatus?.stats?.orderCount || 0, i: "🛒", c: C.green }, { l: "Bekleyen", v: operatorStatus?.stats?.pendingRecs || 0, i: "⏳", c: C.yellow }].map((s, idx) => (
                                ...
                                {/* Mode */}
                                <GlassCard delay={0.2}>
                                    <SectionTitle icon="🎛️" title="Kontrol Modu" color={modeConfig.color} />
                                ...
                                <GlassCard delay={0.25}>
                                    <SectionTitle icon="🛡️" title="Güvenlik Limitleri" color={C.yellow} />
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                                        {operatorStatus?.guardrails && Object.entries(operatorStatus.guardrails).map(([key, value]) => {
                                            const cfg = { maxPriceChangePercent: { l: "Max Fiyat Değişimi", i: "💰", u: "%", c: C.yellow }, maxStockOrderQuantity: { l: "Max Stok Siparişi", i: "📦", u: " adet", c: C.blue }, minProfitMarginPercent: { l: "Min Kâr Marjı", i: "📈", u: "%", c: C.green }, maxActionsPerHour: { l: "Max Aksiyon/Saat", i: "⚡", u: "", c: C.accent }, requireApprovalForCritical: { l: "Kritik Onay", i: "🔒", u: "", c: C.red }, cooldownMinutes: { l: "Cooldown", i: "⏱️", u: " dk", c: C.purple } };
                                            const cc = cfg[key] || { l: key, i: "⚙️", u: "", c: C.dim };
                                            return (
                                                <div key={key} style={{ background: `${cc.c}06`, border: `1px solid ${cc.c}15`, borderRadius: 12, padding: "0.7rem 0.8rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                                    ...
                                                    <div style={{ flex: 1 }}>
                                                        ...
                                                        <div style={{ color: cc.c, fontSize: "0.95rem", fontWeight: 800 }}>{typeof value === "boolean" ? (value ? "✅ Evet" : "❌ Hayır") : `${value}${cc.u}`}</div>
                                                    </div>
                                                </div>
                                            );
                                        })}
                            ...
                            {/* Quick Commands */}
                            <GlassCard delay={0.3}>
                                <SectionTitle icon="⚡" title="Hızlı Komutlar" color={C.accent} />
                                <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                                    {[
                                        { label: "Genel Durum", msg: "Nasıl gidiyor?", icon: "📊", color: C.accent },
                                        { label: "Satış Analizi", msg: "Satışlarım nasıl?", icon: "💰", color: C.green },
                                        { label: "Stok Kontrolü", msg: "Stok durumu nedir?", icon: "📦", color: C.blue },
                                        { label: "Sorun Tespiti", msg: "Sorunları göster", icon: "🚨", color: C.red },
                                        { label: "Öneriler", msg: "Ne yapmalıyım?", icon: "💡", color: C.yellow },
                                        { label: "Pazaryeri Karşılaştır", msg: "Pazaryerlerini karşılaştır", icon: "🏪", color: C.purple },
                                        { label: "Kâr Analizi", msg: "Kâr marjlarım nasıl?", icon: "📈", color: C.pink },
                                        { label: "Tahmin", msg: "Önümüzdeki hafta tahmini", icon: "🔮", color: C.accentAlt },
                                    ].map((cmd, i) => (
                    ...
                    {activeTab === "alerts" && (
                        <motion.div key="al" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                            <GlassCard delay={0.1}>
                                <SectionTitle icon="🚨" title="Proaktif Uyarılar" badge={`${alerts.length} uyarı`} color={criticalAlerts.length > 0 ? C.red : C.green} />
                                {alerts.length === 0 ? (
                                    <div style={{ textAlign: "center", padding: "3rem 1rem" }}>
                                        ...
                                        <p style={{ color: C.green, fontSize: "1.1rem", fontWeight: 800, margin: 0, textShadow: `0 0 20px ${C.green}40` }}>Tüm sistemler normal!</p>
                                        <p style={{ color: C.dim, fontSize: "0.82rem", margin: "0.4rem 0 0" }}>Kritik uyarı bulunmuyor.</p>
                                    </div>
                                ) : (
                                    <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                                        {alerts.map((alert, i) => {
                                                ...
                                                <motion.div key={i} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                                                    ...
                                                    <div style={{ flex: 1 }}>
                                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.25rem" }}>
                                                            ...
                                                            <Pill color={sc}>{alert.severity === "critical" ? "Kritik" : alert.severity === "high" ? "Yüksek" : "Bilgi"}</Pill>
                                                        </div>
                    ...
                    {activeTab === "memory" && (
                        <motion.div key="mem" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1.25rem" }}>
                                <GlassCard delay={0.1}>
                                    <SectionTitle icon="🧬" title="AI Hafıza İstatistikleri" color={C.purple} />
                                    <ProgressBar label="Toplam Hafıza" value={operatorStatus?.memory?.totalMemories || 0} max={200} color={C.accentAlt} sublabel={`${operatorStatus?.memory?.totalMemories || 0} kayıt`} />
                                    <ProgressBar label="Aksiyon Sonuçları" value={operatorStatus?.memory?.actionMemories || 0} max={100} color={C.green} sublabel={`${operatorStatus?.memory?.actionMemories || 0} sonuç`} />
                                    <ProgressBar label="Öğrenilen Patternler" value={operatorStatus?.memory?.learnedPatterns || 0} max={50} color={C.yellow} sublabel={`${operatorStatus?.memory?.learnedPatterns || 0} pattern`} />
                                    <ProgressBar label="Kullanıcı Tercihleri" value={operatorStatus?.memory?.userPreferences || 0} max={30} color={C.purple} sublabel={`${operatorStatus?.memory?.userPreferences || 0} tercih`} />
                                    <ProgressBar label="Pazar İçgörüleri" value={operatorStatus?.memory?.marketInsights || 0} max={50} color={C.blue} sublabel={`${operatorStatus?.memory?.marketInsights || 0} içgörü`} />
                                </GlassCard>
                                <GlassCard delay={0.15}>
                                    <SectionTitle icon="📋" title="Son AI Aksiyonları" color={C.green} />
                                    {operatorStatus?.memory?.recentActions?.length > 0 ? (
                                        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                            {operatorStatus.memory.recentActions.map((a, i) => (
                                                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                                                    ...
                                                    <Pill color={a.success ? C.green : C.red}>{a.success ? "Başarılı" : "Başarısız"}</Pill>
                                                </motion.div>
                                            ))}
                                        </div>
                                    ) : (
                                        <div style={{ textAlign: "center", padding: "2.5rem 1rem" }}>
                                            ...
                                            <p style={{ color: C.dim, fontSize: "0.88rem", margin: 0, fontWeight: 600 }}>Henüz aksiyon kaydı yok</p>
                                            <p style={{ color: C.dim, fontSize: "0.72rem", margin: "0.3rem 0 0" }}>Döngü çalıştırarak AI'ın öğrenmesini başlatın</p>
                                        </div>
                                    )}
                                </GlassCard>
                                <GlassCard delay={0.2}>
                                    <SectionTitle icon="📚" title="Öğrenme Döngüsü" color={C.pink} />
                                    <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                                        {[
                                            { icon: "🔄", title: "Otonom Döngü", desc: "6 fazlı döngü: Gözlem → Analiz → Karar → Aksiyon → Doğrulama → Öğrenme", color: C.accent },
                                            { icon: "📊", title: "Veri Toplama", desc: "Ürünler, siparişler, pazaryeri verileri sürekli izleniyor", color: C.blue },
                                            { icon: "🧠", title: "Pattern Tanıma", desc: "Başarılı/başarısız aksiyonlar hafızaya kaydediliyor", color: C.purple },
                                            { icon: "📈", title: "Sürekli İyileşme", desc: "Her döngüde AI daha iyi kararlar almayı öğreniyor", color: C.green },
                                            { icon: "🛡️", title: "Güvenlik Katmanı", desc: "Guardrail limitleri aşılamaz, kritik aksiyonlar onay gerektirir", color: C.yellow },
                                        ].map((item, i) => (
                            ...
                            <GlassCard delay={0.1} style={{ padding: 0, overflow: "hidden" }} glow={C.purpleGlow}>
                                <div style={{ background: `linear-gradient(135deg, ${C.accentAlt}12, ${C.accent}08)`, borderBottom: `1px solid ${C.glassBr}`, padding: "1rem 1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                                        ...
                                        <div>
                                            <span style={{ color: "#fff", fontSize: "0.95rem", fontWeight: 800 }}>AI Operatör Sohbet</span>
                                            <div style={{ display: "flex", alignItems: "center", gap: "0.35rem" }}>
                                                ...
                                                <span style={{ color: C.muted, fontSize: "0.7rem" }}>Çevrimiçi — Neural Network Aktif</span>
                                            </div>
                                        </div>
                                    </div>
                                    <NeonButton color={C.muted} size="sm" onClick={() => setChatMessages([])}>🗑️ Temizle</NeonButton>
                                </div>
                                <div style={{ height: 480, overflowY: "auto", padding: "1rem 1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem", background: `linear-gradient(180deg, rgba(5,10,18,0.5) 0%, rgba(5,10,18,0.8) 100%)` }}>
                                    {chatMessages.length === 0 && (
                                        <div style={{ textAlign: "center", padding: "2.5rem 1rem" }}>
                                            ...
                                            <p style={{ color: C.muted, fontSize: "0.85rem", margin: "0.4rem 0 1.2rem" }}>Ben LysiaETIC AI Operatör. İşletmenizi yönetmek için buradayım.</p>
                                            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem", justifyContent: "center" }}>
                                                {["Nasıl gidiyor?", "Satışlarım nasıl?", "Stok durumu", "Ne yapmalıyım?"].map((s, i) => (
                                ...
                                <div style={{ padding: "0.8rem 1.25rem", borderTop: `1px solid ${C.glassBr}`, display: "flex", gap: "0.5rem", alignItems: "flex-end", background: "rgba(5,10,18,0.5)" }}>
                                    <textarea ref={chatInputRef} value={chatInput} onChange={(e) => setChatInput(e.target.value)}
                                        ...
                                        placeholder="AI Operatör'e mesaj yazın..." disabled={chatLoading} rows={1}
                    ...
                    {activeTab === "cycle" && (
                        <motion.div key="cy" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                            {cycleResult ? (<>
                                <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
                                    {[{ l: "AI Skor", v: cycleResult.analysis?.aiScore?.overall || 0, c: C.accentAlt, i: "🎯" }, { l: "Kararlar", v: cycleResult.decisions?.decisions?.length || 0, c: C.yellow, i: "🧠" }, { l: "Uygulanan", v: cycleResult.actionResults?.length || 0, c: C.green, i: "✅" }, { l: "Kritik", v: cycleResult.decisions?.criticalCount || 0, c: C.red, i: "🚨" }, { l: "Süre", v: `${cycleResult.durationMs || 0}`, c: C.blue, i: "⏱️", s: "ms" }].map((s, i) => (
                                ...
                                {cycleResult.analysis?.focusItems?.length > 0 && (
                                    <GlassCard delay={0.15} style={{ marginBottom: "1.25rem" }}>
                                        <SectionTitle icon="🎯" title="Öncelikli Konular" color={C.yellow} />
                                        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                ...
                                {cycleResult.decisions?.decisions?.length > 0 && (
                                    <GlassCard delay={0.2}>
                                        <SectionTitle icon="🤖" title="AI Kararları" badge={`${cycleResult.decisions.decisions.length} karar`} color={C.accentAlt} />
                                ...
                                <GlassCard delay={0.1}>
                                    <div style={{ textAlign: "center", padding: "3rem 1rem" }}>
                                        ...
                                        <p style={{ color: "#fff", fontSize: "1.1rem", fontWeight: 800, margin: 0 }}>Henüz döngü çalıştırılmadı</p>
                                        <p style={{ color: C.dim, fontSize: "0.85rem", margin: "0.4rem 0 1.2rem" }}>Yukarıdaki "Döngü Çalıştır" butonuna tıklayarak AI'ın tam analiz döngüsünü başlatın.</p>
                                        <NeonButton color={C.accent} onClick={runCycle} disabled={cycleLoading} size="lg">▶️ Şimdi Çalıştır</NeonButton>
                                    </div>
                                </GlassCard>
                            )}
                        </motion.div>
