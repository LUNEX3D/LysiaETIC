...
const AnalyticsPage = () => {
    ...
    const loadMarketplaces = async () => {
        try {
            ...
            console.error("Pazaryerleri yüklenemedi:", error);
        }
    };
    ...
    const loadAnalyticsData = async () => {
        ...
        try {
            ...
            console.log("📊 Analytics verileri yükleniyor...", params);
            ...
            if (overviewRes.data.success && overviewRes.data.data) {
                console.log("📊 KPI Data:", overviewRes.data.data);
                setKpiData(overviewRes.data.data);
            } else {
                console.warn("⚠️ KPI data yok veya başarısız");
            }

            // Set Sales Trend
            if (trendRes.data.success && trendRes.data.data) {
                console.log("📈 Sales Trend Data:", trendRes.data.data.length, "günlük veri");
                ...
                console.log("✅ Sales trend set edildi:", formattedTrend.length, "gün");
            } else {
                console.warn("⚠️ Sales trend data yok");
                setSalesTrend([]);
            }

            // Set Marketplace Distribution
            if (distributionRes.data.success && distributionRes.data.data) {
                console.log("🏪 Marketplace Data:", distributionRes.data.data);
                ...
                console.warn("⚠️ Marketplace distribution data yok");
            }

            // Set Top Products
            if (topProductsRes.data.success && topProductsRes.data.data) {
                console.log("🔥 Top Products Data:", topProductsRes.data.data.length, "ürün");
                ...
                console.log("✅ Top products set edildi:", formattedProducts.length, "ürün");
            } else {
                console.warn("⚠️ Top products data yok");
                setTopProducts([]);
            }

            // Set Category Distribution
            if (categoryRes.data.success && categoryRes.data.data) {
                console.log("📊 Category Data:", categoryRes.data.data.length, "kategori");
                setCategoryData(categoryRes.data.data);
                console.log("✅ Category data set edildi:", categoryRes.data.data.length, "kategori");
            } else {
                console.warn("⚠️ Category data yok");
                setCategoryData([]);
            }

            // Set Hourly Sales
            if (hourlyRes.data.success && hourlyRes.data.data) {
                console.log("⏰ Hourly Data:", hourlyRes.data.data.length, "saat");
                setHourlyData(hourlyRes.data.data);
                console.log("✅ Hourly data set edildi:", hourlyRes.data.data.length, "saat");
            } else {
                console.warn("⚠️ Hourly data yok");
                setHourlyData([]);
            }

            console.log("✅ Tüm analytics verileri yüklendi");

        } catch (error) {
            console.error("❌ Analytics data yüklenemedi:", error);
